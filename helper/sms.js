...
smsHelper.sendOTP = async (data) => {
    try {
        ...
        const otp_sms = "Otp for verify your mobile number is " + otp_number + " do not share this otp with others.";
        const otp = otp_number;

        const resp = await nexmo.message.sendSms(from, to, otp_sms);

        console.log("sms respone =>",resp);
        // console.log("mobile =>",send_to);
        // console.log("otp_sms =>",otp_sms);
        // console.log("otp =>",otp);
